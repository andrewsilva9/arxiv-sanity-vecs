...
def get_seen(dict_back=False):
    ...
    if dict_back:
        return seen_papers
    seen = seen_papers['liked'] + seen_papers['disliked']
    return seen
...
def render_pid(pid):
    ...
    return dict(
        ...
        summary=d['summary'],
        thumb_url=thumb_url,
        liked=pid in seen['liked'],
        disliked=pid in seen['disliked']
    )
...
def dot_product_rank(pid=None, do_time_filter='', do_have_filter=''):
    ...
    with open('data/sentence_features.pt', 'rb') as f:
    ...
    papers = []
    scores = []
    for x in paper_orders:
        papers.append(pids[x['corpus_id']])
...
def main():
    ...
    opt_rank = request.args.get('rank', default_rank)  # rank type. search|tags|embed|pid|time|random
...
@app.route('/add_embed/<pid>')
def add_embed(pid=None):
    if g.user is None:
        return "error, not logged in"
    move_for_embed(pid=pid, move_away=False)
    return "ok"


@app.route('/sub_embed/<pid>')
def sub_embed(pid=None):
    if g.user is None:
        return "error, not logged in"
    move_for_embed(pid, move_away=True)
    return "ok"


def move_for_embed(pid, move_away: bool = False):
    with open('data/sentence_features.pt', 'rb') as f:
        features = pickle.load(f)
    x, pids = features['x'], features['pids']
    paper_features = x[pids.index(pid)]
    user_data = load_users()
    user_embed = user_data[session['user']]
    if move_away:
        user_embed -= ALPHA * (paper_features - user_embed)
    else:
        user_embed += ALPHA * (paper_features - user_embed)
    user_data[session['user']] = user_embed
    save_users(user_data)
    print(f'Moved {g.user} closer to paper {pid}')
    with get_seen_db(flag='c') as seen_db:

        # create the user if we don't know about them yet with an empty library
        if not g.user in seen_db:
            seen_db[g.user] = {'liked': [], 'disliked': []}

        # fetch the user library object
        d = seen_db[g.user]
        # add the paper to the tag
        if move_away and pid not in d['disliked']:
            d['disliked'].append(pid)
        elif pid not in d['liked']:
            d['liked'].append(pid)
        # write back to database
        seen_db[g.user] = d
    return "ok"
