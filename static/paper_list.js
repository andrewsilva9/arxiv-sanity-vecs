...
const Paper = props => {
    ...
    const add_embed = () => fetch("/add_embed/" + p.id).then(response=>console.log(response.text()))
    const sub_embed = () => fetch("sub_embed/" + p.id).then(response=>console.log(response.text()))
